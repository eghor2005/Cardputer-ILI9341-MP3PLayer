...
M5Canvas sprite(&externalDisplay);
M5Canvas spr(&externalDisplay);
...
void setup() {
  ...
  externalDisplay.setRotation(3);  // Настройте поворот
  externalDisplay.setBrightness(255);
  externalDisplay.fillScreen(BLACK);
  Serial.println("External display initialized");


  sprite.createSprite(320, 240);   // Полный экран
  spr.createSprite(116, 16);   
...
void draw() {
  if (graphSpeed == 0) {
    ...
    // Основной фон
    sprite.fillRect(0, 0, 320, 240, gray);  // 320x240 вместо 240x135
    
    // Список файлов (сдвинуто влево, т.к. экран шире)
    sprite.fillRect(4, 8, 180, 182, BLACK);  // Шире список
    sprite.fillRect(179, 8, 5, 182, 0x0841);
    
    // Слайдер прокрутки
    sliderPos = map(n, 0, fileCount, 8, 170);
    sprite.fillRect(179, sliderPos, 5, 20, grays[2]);
    sprite.fillRect(181, sliderPos + 4, 1, 12, grays[16]);
    
    // Декоративные линии
    sprite.fillRect(4, 2, 50, 2, ORANGE);
    sprite.fillRect(84, 2, 50, 2, ORANGE);
    sprite.fillRect(230, 2, 85, 2, ORANGE);
    sprite.fillRect(230, 6, 85, 3, grays[4]);
    
    // Рамки
    sprite.drawFastVLine(3, 9, 180, light);
    sprite.drawFastVLine(184, 9, 180, light);
    sprite.drawFastHLine(3, 189, 180, light);
    sprite.drawFastHLine(0, 0, 320, light);
    sprite.drawFastHLine(0, 199, 320, light);
    
    // Разделитель для правой панели
    sprite.fillRect(189, 0, 3, 200, BLACK);
    sprite.drawFastVLine(188, 0, 200, light);
    sprite.drawFastVLine(317, 0, 200, light);
    
    // Область отображения трека (увеличена)
    sprite.fillRect(196, 14, 116, 42, BLACK);
    sprite.fillRect(196, 59, 116, 16, BLACK);
    
    // Кнопки управления (4 штуки)
    for (int i = 0; i < 4; i++)
      sprite.fillRoundRect(196 + (i * 28), 94, 24, 24, 3, grays[4]);
    
    // Иконки кнопок
    sprite.fillRect(290, 104, 10, 3, grays[13]);
    sprite.fillRect(290, 110, 10, 3, grays[13]);
    sprite.fillTriangle(300, 101, 300, 106, 303, 105, grays[13]);
    sprite.fillTriangle(290, 106, 290, 110, 287, 109, grays[13]);
    
    // Кнопка Play/Stop
    if (!stoped) {
      sprite.fillRect(204, 104, 4, 8, grays[13]);
      sprite.fillRect(211, 104, 4, 8, grays[13]);
    } else {
      sprite.fillTriangle(206, 101, 206, 110, 211, 106, grays[13]);
    }
    
    // Полоса громкости
    sprite.fillRoundRect(220, 82, 80, 3, 2, YELLOW);
    sprite.fillRoundRect(200 + ((volume / 5) * 17), 79, 12, 10, 2, grays[2]);
    sprite.fillRoundRect(202 + ((volume / 5) * 17), 81, 8, 6, 2, grays[10]);
    
    // Полоса яркости
    sprite.fillRoundRect(220, 124, 50, 3, 2, MAGENTA);
    sprite.fillRoundRect(220 + (bri * 10), 121, 14, 10, 2, grays[2]);
    sprite.fillRoundRect(222 + (bri * 10), 123, 10, 6, 2, grays[10]);
    
    // Индикатор батареи
    sprite.drawRect(270, 119, 40, 14, GREEN);
    sprite.fillRect(310, 122, 4, 8, GREEN);
    ...
    for (int i = 0; i < 14; i++) {
      if (!stoped)
        g[i] = random(1, 5);
      for (int j = 0; j < g[i]; j++)
        sprite.fillRect(220 + (i * 6), 50 - j * 3, 4, 2, grays[4]);
    }
    ...
    if (n < 5) {
      for (int i = 0; i < 10; i++) {
        ...
        if (i < fileCount)
          sprite.drawString(audioFiles[i].substring(1, 25), 8, 10 + (i * 12));
      }
    } else {
      int yos = 0;
      for (int i = n - 5; i < n - 5 + 10; i++) {
        ...
        if (i < fileCount)
          sprite.drawString(audioFiles[i].substring(1, 25), 8, 10 + (yos * 12));
        yos++;
      }
    }
    ...
    sprite.drawString("WINAMP", 198, 4);
    sprite.setTextColor(grays[2], gray);
    sprite.drawString("LIST", 58, 0);
    sprite.setTextColor(grays[4], gray);
    sprite.drawString("VOL", 196, 80);
    sprite.drawString("LIG", 196, 122);
    
    // Статус PLAY/STOP
    if (isPlaying) {
      sprite.setTextColor(grays[8], BLACK);
      sprite.drawString("P", 200, 18);
      sprite.drawString("L", 200, 27);
      sprite.drawString("A", 200, 36);
      sprite.drawString("Y", 200, 45);
    } else {
      sprite.setTextColor(grays[8], BLACK);
      sprite.drawString("S", 200, 18);
      sprite.drawString("T", 200, 27);
      sprite.drawString("O", 200, 36);
      sprite.drawString("P", 200, 45);
    }
    ...
    if (!stoped)
      sprite.drawString(rtc.getTime().substring(3, 8), 220, 18);
    
    // Батарея в процентах
    sprite.setTextFont(0);
    int percent = getBatteryPercent();
    sprite.setTextDatum(3);
    sprite.drawString(String(percent) + "%", 288, 121);
    
    // Подписи кнопок
    sprite.setTextColor(BLACK, grays[4]);
    sprite.drawString("B", 288, 96);
    sprite.drawString("N", 258, 96);
    sprite.drawString("P", 228, 96);
    sprite.drawString("A", 198, 96);
    
    sprite.setTextColor(BLACK, grays[5]);
    sprite.drawString(">>", 264, 103);
    sprite.drawString("<<", 234, 103);
    ...
    textPos = textPos - 2;
    if (textPos < -400) textPos = 100;  // Больше места для прокрутки
    spr.pushSprite(&sprite, 196, 59);
    
    // Выводим на экран
    sprite.pushSprite(&externalDisplay, 0, 0);
  }
